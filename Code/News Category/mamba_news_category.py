# -*- coding: utf-8 -*-
"""Mamba_News_Category_self_iter.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xlagXeOSK8fGioDckgJPUnQ68Vh2OUFS
"""
...
avg_words = df['short_description'].str.split().str.len().mean()
print(f"short_description 平均詞數：{avg_words:.2f}")

avg_words = df['headline'].str.split().str.len().mean()
print(f"headline 平均詞數：{avg_words:.2f}")
...
# 建立「headline 空」的布林遮罩
mask_empty = df['headline'].isna() | (df['headline'].str.strip() == '')

# 顯示這些 row，檢查 short_description、category…等欄位是否有值
empty_rows = df[mask_empty]
print(f"共 {len(empty_rows)} 筆 headline 為空：")
display(empty_rows)

def run_experiment(vocab_size, max_length, batch_size, embedding_dim, hidden_size, conv_kernel_size, num_layers, dropout, save_dir, epochs=10):
    class StackedMambaClassifierMulti(nn.Module):
        def __init__(
            self,
            vocab_size: int,
            num_classes: int,
            ...
            self.mamba_layers = nn.ModuleList([
                Mamba(d_model=emb_dim, d_state=hidden_size, d_conv=conv_kernel_size)
        ...
        def forward(self, x: torch.LongTensor) -> torch.Tensor:
            ...
            for layer in self.mamba_layers:
                x = layer(x)                  # each: (batch_size, seq_len, emb_dim)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_val), start=1):
        print(f"\n========== Fold {fold} ===========")
        model     = StackedMambaClassifierMulti(vocab_size, num_classes, embedding_dim, hidden_size, conv_kernel_size).to(device)
...
folder_name = f"exp_single_vs{vocab_size}_ml{max_length}_bs{batch_size}_ed{embedding_dim}_hs{hidden_size}_ks{conv_kernel_size}"
base_path   = "/content/drive/MyDrive/Project--code/News/Mamba_outcome_1"
save_dir    = os.path.join(base_path, folder_name)
os.makedirs(save_dir, exist_ok=True)

run_experiment(vocab_size, max_length, batch_size, embedding_dim, hidden_size, conv_kernel_size, num_layers, dropout, save_dir, epochs=10)
...
for i, (num_layers, vocab_size, max_length, batch_size, embedding_dim, hidden_size, conv_kernel_size) in enumerate(param_combinations, start=1):
    print(f"\n===== Running Experiment {i}/{len(param_combinations)} =====")
    folder_name = f"exp_{i}_vs{vocab_size}_ml{vocab_size}_bs{batch_size}_ed{embedding_dim}_hs{hidden_size}_ks{conv_kernel_size}"
    base_path   = "/content/drive/MyDrive/Project--code/News/Mamba_outcome_2"
    save_dir    = os.path.join(base_path, folder_name)
    os.makedirs(save_dir, exist_ok=True)
    run_experiment(vocab_size, max_length, batch_size, embedding_dim, hidden_size, conv_kernel_size, num_layers, dropout, save_dir, epochs=10)
